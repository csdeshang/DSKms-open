...
class  Voucher extends BaseModel
{
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $pagesize = null)
    {
        ...
        if($pagesize){
        $list = Db::name('voucher')->alias('v')->join('store s','s.store_id=v.voucher_store_id')->join('vouchertemplate t','v.vouchertemplate_id=t.vouchertemplate_id')->where($where)->order('voucher_enddate asc')->paginate(['list_rows'=>$pagesize,'query' => request()->param()],false);
        $this->page_info=$list;
        $list=$list->items();
        }else{
            $list = Db::name('voucher')->alias('v')->join('store s','s.store_id=v.voucher_store_id')->join('vouchertemplate t','v.vouchertemplate_id=t.vouchertemplate_id')->where($where)->order('voucher_enddate asc')->select()->toArray();
        }
    ...
    public function editVoucherState($voucher_list)
    {
        ...
        foreach ($voucher_list as $store_id => $voucher_info) {
            $update = $this->editVoucher(array('voucher_state' => 2,'voucher_order_id' =>$voucher_info['order_id'] ), array('voucher_id' => $voucher_info['voucher_id']), $voucher_info['voucher_owner_id']);
            if ($update) {
                // 发送用户机构消息
