...
class  Pointprod extends MobileMall {
    ...
    public function plist() {
        ...
        $pageSize = 10;
        $pointprod_list = $pointprod_model->getPointProdList($condition, "*", $orderby, '', $pageSize);
        $page_count = $pointprod_model->page_info;

        
        $result = array_merge(array('goods_list' => $pointprod_list, 'grade_list' => $membergrade_arr, 'ww' => json_encode($condition)), mobile_page($page_count));
    ...
    public function get_order_list(){
        ...
        $condition[] = array('pointog_goodsid','=',$pgoods_id);
        $orderprod_list = $pointorder_model->getPointorderAndGoodsList($condition, '*',  'pointsordergoods.pointog_recid desc',$this->pagesize);
        if ($orderprod_list) {
            $buyerid_arr = array();
