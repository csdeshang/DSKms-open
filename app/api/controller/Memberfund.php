...
class  Memberfund extends MobileMember {
    ...
    public function predepositlog() {
        ...
        $condition = array();
        $condition[] = array('lg_member_id','=',$this->member_info['member_id']);
        $condition[] = array('lg_av_amount','<>',0);
        
        $lg_type = input('param.lg_type');
        if(array_key_exists($lg_type,$predeposit_model->lg_type_text)){
            $condition[] = array('lg_type', '=', $lg_type);
        }
        
        $begin_time = input('param.begin_time');
        $end_time = input('param.end_time');
        if ($begin_time) {
            $condition[] = array('lg_addtime','>=',$begin_time);
        }
        if ($end_time) {
            $condition[] = array('lg_addtime','<=',$end_time + 24 * 3600);
        }
        
        $list = $predeposit_model->getPdLogList($condition, $this->pagesize, '*', 'lg_id desc');
        if ($list) {
            foreach ($list as $k => $v) {
