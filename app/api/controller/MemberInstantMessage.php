    ...
    public function add() {
        ...
        switch ($to_type) {
            ...
            case 2:
                ...
                $to_name = $live_apply['live_apply_id'] . '号直播间';
                if ($live_apply['live_apply_type_id']) {
                    switch ($live_apply['live_apply_type']) {
                        case LIVE_APPLY_TYPE_COURSE:
                            $goodscourse=model('goodscourse')->getOneGoodscourses(array('goodscourse_id'=>$live_apply['live_apply_type_id']));
                            if(!$goodscourse){
                                ds_json_encode(10001, '课程章节不存在');
                            }
                            $goods=model('goods')->getGoodsOnlineInfoByID($goodscourse['goods_id']);
                            if(!$goods){
                                ds_json_encode(10001, '课程不存在');
                            }
                            $to_name=$goods['goods_name'].'：'.$goodscourse['goodscourse_name'];
                            break;
                    }
                }
                break;
        ...
        ds_json_encode(10000, config('ds_config.instant_message_verify') ? '消息已发送，管理员审核后才能显示' : '发送成功', array('instant_message_id' => $instant_message_id, 'instant_message_verify' => config('ds_config.instant_message_verify')));
    }
