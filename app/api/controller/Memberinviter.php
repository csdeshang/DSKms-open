...
class  Memberinviter extends MobileMember {
    ...
    public function check() {
        ...
        if (!$inviter_info) {
            ...
            if (config('ds_config.inviter_condition')) {
                //检查消费金额
                $temp = Db::name('vrorder')->where('buyer_id=' . $this->member_info['member_id'] . ' AND order_state=' . ORDER_STATE_PAY)->field('SUM(order_amount) AS order_amount,SUM(refund_amount) AS refund_amount')->find();
    ...
    public function order() {
        ...
        $list = Db::name('orderinviter')->where($condition)->order('orderinviter_id desc')->paginate(10, false, ['query' => request()->param()]);
