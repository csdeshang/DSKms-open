...
class  Member extends MobileMember {
    ...
    public function index() {
        ...
        $member_info['order_nopay_count'] = $vrorder_model->getVrorderCount($condition);//未支付
        $condition = array(
            'buyer_id'=>$this->member_info['member_id'],
            'order_state'=>ORDER_STATE_SEND
        );
        $member_info['order_noreceipt_count'] = $vrorder_model->getVrorderCount($condition);//未收货
        $condition = array(
            'buyer_id'=>$this->member_info['member_id'],
        ...
        $member_info['order_noeval_count'] = $vrorder_model->getVrorderCount($condition);//未评价
        $condition = array(
            'buyer_id'=>$this->member_info['member_id'],
            'order_state'=>ORDER_STATE_PAY
        );
        $member_info['order_noship_count'] = $vrorder_model->getVrorderCount($condition);//未签收
        
        $vrrefund_model = model('vrrefund');
        $member_info['order_refund_count'] = intval($vrrefund_model->getVrrefundCount(array('buyer_id' => $this->member_info['member_id'], 'admin_state' => 1)));
    ...
    public function goods_poster() {
        ...
        if (!file_exists($refer_qrcode_logo) || 1) {
                ...
                //商品图片
                $goods_image = goods_cthumb($goods_detail['goods_info']['goods_image'],1260);
